...
def execute_scripts():
    ...
    if not st.session_state.execute_flag:
        ...
        time.sleep(7)
        st.session_state.execute_flag = True  # Set the flag after starting
...
if __name__ == "__main__":
    ...
    with st.sidebar:
        ...
        if st.button("Start Execution"):
            if not st.session_state.execute_flag:  # Check before calling
                execute_scripts()

        if st.button("Stop Execution"):  # Add a button to stop
            st.session_state.execute_flag = False
