...
# Load your time series data (assuming you already have 'df_time_series' loaded)
# ...

# Preprocessing
# df_time_series = pd.read_csv('../data/time_series_data.csv', index_col='DATE', parse_dates=True)
# For demonstration, let's create some dummy data if you haven't loaded it yet
np.random.seed(42)
...
class TransformerModel(nn.Module):
    ...
    def forward(self, src):
        ...
        if src.shape[0] != sequence_length:  # changed from self.sequence_length
...
model.eval()
with torch.no_grad():
    last_sequence = torch.tensor(scaled_data[-sequence_length:]).float().unsqueeze(0)
